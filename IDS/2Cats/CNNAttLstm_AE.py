...
from sklearn.metrics import recall_curve as recall
...
Xy_test = np.concatenate((X_train, label_train), axis = 1)
Xy_train = np.concatenate((X_test, label_test), axis = 1)
...
class cnn_lstm(nn.Module):
    ...
    def forward(self, x):
        ...
        aligned = x.mean(-1)
        x, (_,_) = self.lstm(aligned.unsqueeze(1))
        x = self.fc4s(x.squeeze(1))
        return x

clstm = cnn_lstm()

state_dict = torch.load('cnnatt_weights.pth', map_location=torch.device('cpu'))
model_dict = clstm.state_dict()
pretrained_dict = {k: v for k, v in state_dict.items() if k in model_dict}
model_dict.update(pretrained_dict)
clstm.load_state_dict(model_dict)
...
optimizer = torch.optim.Adam(clstm.parameters(), lr=learningRate)
...
with torch.no_grad():
    for x in test_loader:
        ...
        # get output from the model, given the inputs
        outputs = clstm(x_.unsqueeze(1))
        _, predicted = torch.max(outputs.data, 1) # arg_max(output)
        total += y_.size(0)
        correct += (predicted == y_).sum().item()
        predictions.extend(predicted.detach().cpu())
        labels.extend(y_.cpu())

maxcor = correct
print(100 * correct / total)
prec = precision(predictions, labels, average='weighted')
reca = recall(predictions, labels, average='weighted')
fsco = fscore(predictions, labels, average='weighted')
...
for epoch in range(epochs):
    ...
    for x in tqdm(train_loader):
        ...
        loss.backward()
        torch.nn.utils.clip_grad_norm_(clstm.parameters(), 1)
        # update parameters
        optimizer.step()
    ...
    if correct > maxcor:
        maxcor = correct
        torch.save(clstm.state_dict(), 'rnn_weights.pth')
    print('Accuracy of the network on test traces: %f %%' % (
        100 * correct / total))
